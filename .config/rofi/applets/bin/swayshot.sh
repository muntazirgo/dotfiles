...
## Script to take screenshots with grim and slurp

time=$(date +%Y-%m-%d-%H-%M-%S)
geometry="$(swaymsg -t get_tree | jq -r '.. | select(.focused?) | .rect | "\(.width)x\(.height)"')"
...
shotwin () {
	cd "${dir}" && grim -g "$(swaymsg -t get_tree | jq -r '.. | select(.focused?) | .rect | "\(.x),\(.y) \(.width)x\(.height)"')" "$file"
	notify_view
}
